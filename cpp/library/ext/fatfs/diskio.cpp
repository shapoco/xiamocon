...
#define CS_HIGH()   { gpio::write(XMC_PIN_TFCARD_CS, 1 ); }
#define CS_LOW()    { gpio::write(XMC_PIN_TFCARD_CS, 0 ); }
...
static void init_spi (void)
{
    ...
    gpio::write(XMC_PIN_TFCARD_CS, 1);
    spi::setBaudrate(spi::getPreferredFrequency(Chipset::MEMORY_CARD));
}

static BYTE rcvr_spi() {
    BYTE buff;
    spi::readBlocking(0xff, &buff, 1);
    return buff;
}
...
{
    spi::readBlocking(0xff, buff, btr);
}


static void xmit_spi(BYTE buff) {
    spi::writeBlocking(&buff, 1);
}
...
{
    CS_HIGH();
    xmit_spi(0xFF); /* Dummy clock (force DO hi-z for multiple slave SPI) */
...
)
{
    ...
    xmit_spi(0xFF); xmit_spi(0xFF);         /* Discard CRC */
...
)
{
    ...
    if (cmd == CMD12) xmit_spi(0xFF);   /* Diacard following one byte when CMD12 */
...
)
{
    ...
    case MMC_GET_OCR:       /* Read OCR (4 bytes) */
        if (send_cmd(CMD58, 0) == 0) {  /* READ_OCR */
            for (n = 0; n < 4; n++) *(((BYTE*)buff) + n) = rcvr_spi();
            res = RES_OK;
        }
        break;
